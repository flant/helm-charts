{{/*
  ...
  | kind: Deployment
  | ...
  |   resources: {{ include "c.resources" (list $ . $.Values.resources) | nindent 4 }}
  ————————————————————————————————————————————————————————————————————
  Results in:
  ————————————————————————————————————————————————————————————————————
  |   resources:
...
{{- define "c.resources" }}
  ...
  {{- range $resourceGroup, $resourceGroupVals := $resources }}
    {{- $resourceGroup | nindent 0 }}:
      {{- $mcpu := include "c.val" (list $ . $resourceGroupVals.mcpu (dict "suffix" "m")) }}
      {{- if $mcpu }}{{ cat "cpu:" $mcpu | nindent 2 }}{{ end }}
      {{- $memoryMb := include "c.val" (list $ . $resourceGroupVals.memoryMb (dict "suffix" "Mi")) }}
