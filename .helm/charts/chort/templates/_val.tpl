{{/*
  ...
  ————————————————————————————————————————————————————————————————————
  | key1: {{ include "c.val" (list $ . $.Values.memory (dict "suffix" "Mi") }}
  ————————————————————————————————————————————————————————————————————
  ...
  | .helm/templates/test.yaml:
  ————————————————————————————————————————————————————————————————————
  | key1: {{ include "c.val" (list $ . $.Values.key1) }}
  ————————————————————————————————————————————————————————————————————
*/}}
{{- define "c.val" }}
  {{- $ := index . 0 }}
  ...
  {{- if kindIs "map" $val }}
    ...
    {{- end }}
    {{- include "c._renderVal" (list $ $relativeScope $currentEnvVal $prefix $suffix) }}
  {{- else }}
    {{- include "c._renderVal" (list $ $relativeScope $val $prefix $suffix) }}
  {{- end }}
{{- end }}
...
{{/*
  Invokes "val" template and if there is a result, then quotes it, otherwise
  no quotes.
*/}}
{{- define "c.valQ" }}
  {{- $result := include "c.val" . }}
  {{- if ne $result "" }}
    {{- $result | quote }}
  {{- end }}
{{- end }}
...
{{/*
  Invokes "val" template and if there is a result, then single quotes it,
  otherwise no quotes.
*/}}
{{- define "c.valSQ" }}
  {{- $result := include "c.val" . }}
