{{/*
  ...
  | kind: Deployment
  | ...
  |   resources: {{ include "flant-lib.generateContainerResources" (list $ . $.Values.resources) | nindent 4 }}
  ————————————————————————————————————————————————————————————————————
  Results in:
  ————————————————————————————————————————————————————————————————————
  |   resources:
...
*/}}
{{- define "flant-lib.generateContainerResources" }}
  {{- $ := index . 0 }}
  ...
  {{- range $resourceGroup, $resourceGroupVals := $resources }}
    {{- $resourceGroup | nindent 0 }}:
      {{- $mcpu := include "flant-lib.value" (list $ . $resourceGroupVals.mcpu (dict "suffix" "m")) }}
      {{- if $mcpu }}{{ cat "cpu:" $mcpu | nindent 2 }}{{ end }}
      {{- $memoryMb := include "flant-lib.value" (list $ . $resourceGroupVals.memoryMb (dict "suffix" "Mi")) }}
